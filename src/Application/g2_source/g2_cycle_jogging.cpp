...
#if 0//obsolete  def DEBUG_JOGGING_RUN_AWAY_DEFECT 
volatile bool jobkill_to_stop_jog_flag=false;
void set_jobkill_to_stop_jog(void){jobkill_to_stop_jog_flag=true;}
#endif
...
stat_t cm_jogging_cycle_start(uint8_t axis) {
    ...
    jog.velocity_max = JOGGING_MAX_VELOCITY;//sme 8-27-2021; replace literal: 2000;//cm->a[axis].velocity_max;
...
{
#ifdef DEBUG_JOGGING_RUN_AWAY_DEFECT
#define MIN_JOG_STEP_COUNT 0
  static int jog_iteration_count=0;
  static int min_jog_step_count = MIN_JOG_STEP_COUNT;
...
#ifdef DEBUG_JOGGING_RUN_AWAY_DEFECT 
     if( cm->job_kill_state==JOB_KILL_JOG_REQUESTED)    
     {
         if (++jog_iteration_count>=min_jog_step_count)
         {
           jog.jog_state =JOG_STATE_STOPPED;
           jog_iteration_count=0;
           //obsolete jobkill_to_stop_jog_flag=false;
          cm_request_job_kill();   
...
#ifdef DEBUG_JOGGING_RUN_AWAY_DEFECT    
    if(jog.jog_state ==JOG_STATE_START)
    {
      jog.jog_state = JOG_STATE_RUNNING;
    }
    else if(jog.jog_state ==JOG_STATE_STOPPED) 
    {
       cm->job_kill_state=JOB_KILL_JOG_REQUESTED;
       last   = 1;//return STAT_OK;
...
static stat_t _jogging_finalize_exit(int8_t axis)  // finish a jog
{
    ...
    cm_canned_cycle_end();    
    comms_mgr_write_msg((char *)"{\"jog\":0}\n");  // needed by OMC jogging function   
    return (STAT_OK);
}
