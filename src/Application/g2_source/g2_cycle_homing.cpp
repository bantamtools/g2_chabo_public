...
#ifdef DEBUG_ZAXIS_HOMING_ANOMALY
...
int8_t homing_get_present_axis(void){return hm.axis;}
bool cycle_homing_active(void){return cm->cycle_type==CYCLE_HOMING;}
float cycle_get_homing_velocity(void){return hm.search_velocity;}
bool atc_homing_cycle_active(void){return (cm->cycle_type== CYCLE_ATC_SEEK);}
 ...
 */  
 GpioHandlerStatusE _homing_handler ( void) 
{
    GpioHandlerStatusE result = GPIO_NOT_HANDLED;  
...
stat_t cm_homing_cycle_start(const float axes[], const bool flags[]) {
    ...
    hm.axis          = AXIS_INIT_UNDEFINED;//sme: use mnemonics, Bubba!-1;                  // set to retrieve initial axis
...
stat_t cm_homing_cycle_callback(void) 
 { volatile bool limit_switch_active_fault=false;
    if (cm->cycle_type != CYCLE_HOMING) 
    {   // exit if not in a homing cycle
#ifdef DEPLOY_LIMIT_SWITCH_FAULT_CHECKING
      limit_switch_active_fault=motion_any_limit_active();
      if (limit_switch_active_fault==true)
      {
         __NOP();
      }
#endif
...
static stat_t _homing_axis_start(int8_t axis) 
{
    ...
    {  // axes are done or error
        if (axis == AXIS_HOMING_DONE) 
        {   
            cm->homing_state = HOMING_HOMED;
            return (_set_homing_func(_homing_finalize_exit));
        }
        else if (axis == AXIS_GET_NEXT_ERROR) 
        {  
            return (_homing_error_exit(AXIS_GET_NEXT_ERROR, STAT_HOMING_ERROR_BAD_OR_NO_AXIS));
        }
    }
    ...
    result=cm->a[axis].travel_max - cm->a[axis].travel_min;
    float travel_distance = std::abs(result) + cm->a[axis].latch_backoff;    
    ...
    hm.homing_input = cm->a[axis].homing_input;
     ...
     /*********************************************************************
      *
      * sme: We are past all the error checking!!! 
      *
      *********************************************************************/
    ...
    status =_set_homing_func(_homing_axis_clear_init);              
    return status;         // perform an initial clear
}

/***********************************************************************************
 * _homing_axis_clear_init() - initiate a clear to move off a switch that is thrown at the start
 *_set_homing_func(_homing_axis_clear);
...
{  volatile stat_t status=STAT_OK;;
   volatile int8_t move_axis=axis;//sme: 5-25-2021 ATC special processing

   if( motion_switch_active(axis) == true)   
   {  // the switch is closed at startup  
        _homing_axis_move(move_axis, -hm.latch_backoff, hm.search_velocity);  // otherwise back off the switch 
   } 
...
{
  int8_t move_axis=axis;//sme: 5-25-2021 ATC special processing 
   _homing_axis_move(move_axis, -hm.latch_backoff, hm.search_velocity);
...
{
   int8_t move_axis=axis;//sme: 5-25-2021 ATC special processing
    _homing_axis_move(move_axis, hm.latch_backoff, hm.latch_velocity);  
...
{
   int8_t move_axis=axis;//sme: 5-25-2021 ATC special processing
    _homing_axis_move(move_axis, hm.zero_backoff, hm.search_velocity);
...
static stat_t _homing_axis_set_position(int8_t axis)
{
    ...
    {  // handle G28.4 cycle - set position to the point of switch closure             
        float contact_position[AXES];      
        kn_forward_kinematics(en_get_encoder_snapshot_vector(), contact_position); 
        _homing_axis_move(axis, contact_position[axis], hm.search_velocity); 
    }
...
static stat_t _homing_axis_move(int8_t axis, float target, float velocity) 
{
    ...
    if (status != STAT_OK) 
    {
        rpt_exception(status,(char*) "Homing move failed. Check min/max settings");
...
static int8_t _get_next_axis(int8_t axis) 
{
    if (axis == AXIS_INIT_UNDEFINED )//sme: use mnemonics, Bubba!!! -1) 
    {  // inelegant brute force solution
        if (hm.axis_flags[AXIS_Z]) 
        {
#ifdef DEBUG_ZAXIS_HOMING_ANOMALY
            //first time homing, force prev_direction to be same as direction wich will be 1
            if (cm->homed[AXIS_Z] != true)
            {
                st_pre.mot[MOTOR_3].prev_direction = 1;
            }
#endif
            return (AXIS_Z);
        }
        ...
        return (AXIS_GET_NEXT_ERROR);//sme: use mnemonics, Bubba!!!-2);  // error
    } 
    else if (axis == AXIS_Z) 
    {
        if (hm.axis_flags[AXIS_X]) 
        {
#ifdef DEBUG_ZAXIS_HOMING_ANOMALY//4-4-2023--also happening to X axis
            //first time homing, force prev_direction to be same as direction wich will be 1
            if (cm->homed[AXIS_X] != true)
            {
                st_pre.mot[MOTOR_1].prev_direction = 1;
            }
#endif             
            return (AXIS_X);
        }
    ...
    return (AXIS_INIT_UNDEFINED);//sme: use mnemonics, Bubba!!! -1);  // done
}
