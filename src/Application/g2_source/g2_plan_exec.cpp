...
#define DEBUG_AVOID_DIV_BY_ZERO//8-8-2022 segment time is INF or NAN when diviser num_segments is 0
...
static void _init_forward_diffs(float v_0, float v_1);
    ...
    volatile static int counts_until_section_is_tail = 0;
    volatile static int exec_aline_feedhold_entry_counts = 0;
    volatile static int exec_aline_feedhold_synch_counts = 0;
#endif 
/****************************************************************************************
 ...
 *
 * Note that there can only be one PLANNED move at a time.
...
stat_t mp_forward_plan()
{
    ...
    if (bf == NULL) 
    {                               // case 0a: NULL means nothing is running - this is OK
        st_prep_null();
        return (STAT_OK);
    }
    if (bf->buffer_state < MP_BUFFER_BACK_PLANNED) 
    { // case 0b: nothing to do. get outta here.
        return (STAT_OK);
    }
...
{    
    ...
    // TODO: Find a better place for this - we shouldn't be concerned with dwells or other move types here,
...
{
  static volatile uint16_t segment_counter=0;
   ...
   { 
    ...
    if (status == STAT_EAGAIN) 
    {
        ...
        segment_counter++;   //sme: debug only          // Note that that'll happen in a lower interrupt level.
    } 
    else 
    {
        segment_counter=0;//reset
        ...
        {
            if (mp_free_run_buffer()==true) // returns true if the buffer is empty otherwise, advances to next unread buffer
            {                 
                if (cm->hold_state == FEEDHOLD_OFF) 
                {
...
*/
    ...
    // E = 0
    // F = Vi
#ifdef DEBUG_AVOID_DIV_BY_ZERO
    if (fp_ZERO(mr->segments)== true)
    {       
      mr->segments=1;
    }
#endif
...
{
    volatile static uint32_t exec_aline_head_entry_count=0;
    exec_aline_head_entry_count++;
    ...
    {                            // INITIALIZATION
        ...
        {
            mr->section = SECTION_BODY;
            return(_exec_aline_body(bf)); 
            // skip ahead to the body generator
        }
...
static stat_t _exec_aline_body(mpBuf_t *bf)
{
  ...
  static uint32_t test_start_time_ms, test_end_time_ms=0; 
  volatile static uint32_t test_delta_time_ms=0;
  volatile static uint32_t test_expected_time_ms = 0; 
  volatile static uint32_t exec_aline_body_entry_count=0;
  exec_aline_body_entry_count++;
  if (mr->section_state == SECTION_NEW) 
    {
        if (fp_ZERO(mr->r->body_length)) 
        {
...
static stat_t _exec_aline_tail(mpBuf_t *bf)
{
    volatile static uint32_t exec_aline_tail_entry_count=0;
    exec_aline_tail_entry_count++;
    if (mr->section_state == SECTION_NEW) 
    {   // INITIALIZATION
...
static stat_t _exec_aline_segment()
{
  ...
  // Set target position for the segment
    ...
    {
        segment_length = (mr->segment_velocity+mr->target_velocity) * 0.5 * mr->segment_time;
        ...
        for (a=0; a<AXES; a++) 
        {
            ...
            to_add = (mr->unit[a] * segment_length) - mr->gm.target_comp[a];
            target = mr->position[a] + to_add;
    ...
    result=mp_set_target_steps(exec_target_steps);
...
{
#ifdef DEPLOY_PROBE_FEEDHOLD_TIMEOUT 
   exec_aline_feedhold_entry_counts++;
#endif
    ...
    {
#ifdef DEPLOY_PROBE_FEEDHOLD_TIMEOUT         
       exec_aline_feedhold_synch_counts++; 
#endif
...
#ifdef DEPLOY_PROBE_FEEDHOLD_TIMEOUT 
#define MIN_COUNTS 2
            //if(counts_until_section_is_tail>MIN_COUNTS)
            {
            exec_aline_feedhold_synch_counts=0;
              counts_until_section_is_tail=0;
              exec_aline_feedhold_entry_counts=0;
            }
#endif
