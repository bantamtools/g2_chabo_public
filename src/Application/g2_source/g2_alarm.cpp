/*
 * g2_alarm.cpp - canonical machine alarm handlers
...
stat_t cm_is_alarmed()
{
    ...
    if (cm1.estop_state != 0)                  
    { return (STAT_COMMAND_REJECTED_BY_SHUTDOWN); }

    if (cm->machine_state == MACHINE_SHUTDOWN) 
    { return (STAT_COMMAND_REJECTED_BY_SHUTDOWN); }
    if (cm->machine_state == MACHINE_PANIC)    
    { return (STAT_COMMAND_REJECTED_BY_PANIC); }
    return (STAT_OK);
}
...
stat_t cm_alarm(const stat_t status, const char *msg)
{
    ...
    rpt_exception(status, (char*)msg);                 // send alarm message
...
stat_t cm_shutdown(const stat_t status, const char *msg)
{
    ...
    { // unhome axes and the machine
        cm->homed[i] = false;
    }
    cm->homing_state = HOMING_NOT_HOMED;

    rpt_exception(status, (char*)msg);                 // send exception report
    sr_request_status_report(SR_REQUEST_TIMED);
    return (status);
}
...
stat_t cm_panic(const stat_t status, const char *msg)
{
    ...
    rpt_exception(status, (char*)msg);                 // send panic report
    return (status);
}
