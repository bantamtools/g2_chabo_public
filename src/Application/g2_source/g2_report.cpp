...
#include "g2_canonical_machine.h"//sme 11-9-2022 
/**** Allocation ****/
srSingleton_t sr;
qrSingleton_t qr;
...
stat_t rpt_exception(stat_t status, /*sme: 10-20-2021 const*/ char *msg)
{
    char *status_msg_ptr= get_status_message(status);//NULL;
...
#define MSG_TRUNCATION_OFFSET 128
          int max_buf_len= EXCP_RPT_MSG_MAX_LEN;
          int truncation_offset=MSG_TRUNCATION_OFFSET;
          char buffer[EXCP_RPT_MSG_MAX_LEN];//EXCP_RPT_MSG_MAX_LEN];
          int len=strlen(msg);
         
          if (len >= max_buf_len)//EXCP_RPT_MSG_MAX_LEN)
          { 
             //truncate the message to prevent memory corruption
             len=(max_buf_len-truncation_offset);//EXCP_RPT_MSG_MAX_LEN-MSG_TRUNCATION_OFFSET);
             msg[len]=0;
           }

           sprintf(buffer, "{\"er\":{\"fb\":%0.2f,\"st\":%d,\"msg\":\"%s - %s\"}}\n",
             CHABO_FIRMWARE_BUILD, status, status_msg_ptr, msg);
...
{
    return(rpt_exception(STAT_GENERIC_EXCEPTION_REPORT, (char*)"bogus exception report")); // bogus exception report for testing
}
...
void sr_init_status_report()
{
    ...
    {
        if (sr_defaults[i][0] == NUL) 
        {
          break; // quit on first blank array entry                   
        }
#define DEFAULT_UNLIKELY_DUMMY_VALUE   -1234567  
        // pre-load values with an unlikely number 
        sr.status_report_value[i] = DEFAULT_UNLIKELY_DUMMY_VALUE;  
        
        // load the index for the SR element
        nv->value_int = nv_get_index((const char *)"", sr_defaults[i]);

        if (nv->value_int == NO_MATCH) 
        {
            rpt_exception(STAT_BAD_STATUS_REPORT_SETTING, (char*)"sr_init_status_report() encountered bad SR setting"); // trap mis-configured profile settings
            return;
        }
...
{
    volatile stat_t status=0;//sme debug only
    uint8_t elements = 0;
    //volatile char *str_ptr=0;
    ...
    for (uint8_t i=0; i<NV_STATUS_REPORT_LEN; i++) 
    {
       ...
       //str_ptr=(char *)&nv->token[0];
       
       if (strcmp(nv->token, "mark") == 0) //sme debug only
       {
          __NOP();//__no_operation();
       }
    ...
    status =_populate_unfiltered_status_report();//sme
    return(status);            // return current values
}
...
stat_t sr_request_status_report(cmStatusReportRequest request_type)
{
    ...
    {  // ignore multiple requests. First one wins.
        return (STAT_OK);
    } 
    ...
    else if (request_type == SR_REQUEST_TIMED) 
    {
        ...
        sr.status_report_systick=sys_get_time_ms(); 
    }
    else 
    {
...
{
    bool time_interval_ripe_flag; 
    bool has_data_flag= false;    
    bool allow_empty  = false;//3-12-2021
    volatile static uint8_t unrequested_status_reports_sent = 0;//3-12-2021
    volatile static uint32_t start_time_of_last_report_written = 0;//3-12-2021
    volatile static uint32_t delta_time_since_last_report_written_ms =0;
    
    time_interval_ripe_flag                 = sys_time_ms_limit_reached(sr.status_report_systick, sr.status_report_interval);
    delta_time_since_last_report_written_ms = sys_get_time_ms()-start_time_of_last_report_written; 
    // conditions where auto generated SRs will not be returned   
    if ((sr.status_report_request == SR_OFF) ||
        (sr.status_report_verbosity == SR_OFF))
    {   
        if ((unrequested_status_reports_sent < 5) 
        &&  (delta_time_since_last_report_written_ms > sr.status_report_interval * 2))
        {
    ...
    //3-12-2021:
    else if ((!time_interval_ripe_flag) || (cs.controller_state != CONTROLLER_READY))
    {
        return (STAT_NOOP);
    }
    else
    ...
    //store a cached version of status_report_request asap
    auto status_report_request_cached = sr.status_report_request;

   // don't send an SR if the planner is experiencing a time constraint
   if (!mp_is_phat_city_time()) 
   {
    ...
    sr.status_report_request = SR_OFF;    
#if 0// sme: 11-16-2022: per observed in g2core latest report.cpp:  
    //       "Don't clear status_report_systick [in order] to avoid race conditions"
    sr.status_report_systick=0;
#endif
    ...
    {      
         has_data_flag=_populate_filtered_status_report() ;
         
         if ((has_data_flag== false)&&(!allow_empty))    // 3-12-2021: we have an empty report loaded
         {  // no new data
              return (STAT_OK);
         }
         else// 3-12-2021
   ...
   //3-12-2021:
   start_time_of_last_report_written = sys_get_time_ms();   
...
static uint8_t _populate_filtered_status_report()
{
    const char sr_str[] = "sr";
    bool has_data = false;
#if 1//sme: 11-9-2022 obsolete debug issue
    bool mark_token_flag=false;//sme 12-1-2020
    static int mark_token_no_data_count =0;
    static int mark_token_has_data_count =0;
#endif
    char tmp[TOKEN_LEN+1];
#ifdef REFACTOR_REPORT
    ...
    for (uint8_t i=0; i<NV_STATUS_REPORT_LEN; i++) 
    {
         ...
         //sme: 11-9-2022 mark_token_flag=false;

        if ((nv->index = sr.status_report_list[i]) == 0) 
        {  // end of list
            break;
        }
...
{ 
  stat_t result=sr_set_status_report(nv);
  return (result); 
}

stat_t sr_get_sv(nvObj_t *nv) { return(get_integer(nv,  (uint8_t &)  sr.status_report_verbosity)); }
stat_t sr_set_sv(nvObj_t *nv) { return(set_int_u8(nv, (uint8_t &) sr.status_report_verbosity, SR_OFF, SR_VERBOSE)); }
...
stat_t qr_queue_report_callback()         // called by controller dispatcher
{
    ...
    if ((qr.queue_report_verbosity == QR_OFF) ||
     ...
     qr.queue_report_requested = false;
#define REPORT_SAFE_LEN 80 //sme
    char report[REPORT_SAFE_LEN];    // we know these reports can't be longer than 30 bytes
...
stat_t qr_get_qv(nvObj_t *nv) { return(get_integer(nv, qr.queue_report_verbosity)); }
stat_t qr_set_qv(nvObj_t *nv) { return(set_int_u8(nv, (uint8_t &) qr.queue_report_verbosity, QR_OFF, QR_TRIPLE)); }
...
{ 
    ...
    {
       sprintf(STDERR_SUBSTITUTE/*cs.out_buf*/, "{\"job\":[%lu,%lu,%lu,%lu]}\n", cfg.job_id[0], cfg.job_id[1], cfg.job_id[2], cfg.job_id[3] );
