...
#include "g2core.h"
#include "g2_util.h"
#include "g2_canonical_machine.h"  // for LAGERs
#include "system_bantam.h"

extern unsigned long sys_get_time_ms(void);
bool FLAGS_NONE[AXES] = { false, false, false };
bool FLAGS_ONE[AXES]  = { true, false, false };
bool FLAGS_ALL[AXES]  = { true, true, true };
...
{
  U32DataU swapped32, temp32;
  temp32.u32_data=raw_32;
  swapped32.bytes[0]=temp32.bytes[3];
  swapped32.bytes[1]=temp32.bytes[2];
  swapped32.bytes[2]=temp32.bytes[1]; 
  swapped32.bytes[3]=temp32.bytes[0];
  
  return swapped32.u32_data;
}/* End function */

//sme:
uint16_t ByteSwap16(uint16_t raw_16)
{
  U16DataU swapped16, temp16;
  temp16.u16_data=raw_16;
  swapped16.bytes[0]=temp16.bytes[1];
  swapped16.bytes[1]=temp16.bytes[0];
  return swapped16.u16_data;
}/* End Function */
/**** String utilities ****
...
#if 0 // old version using sprintf. Does not do trailing zero suppression
...
{
#if 1 //7-18-2022 
    volatile float value=n;//sme: why "inf" on "C" axis stowaway user data? 
...
#else //original strange new "Inf" fault   
    // handle special cases
    if (isnan(n)) {
        strcpy(str, "nan");
        return (3);
