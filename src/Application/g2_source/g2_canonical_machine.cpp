...
#include "g2_kinematics.h"     // for forward kinematics in cm_cycle_start
#include "comms_mgr.h"
#include "system_bantam.h"  

extern void mp_start_feed_override(/*const*/ float ramp_time, /*const*/ float override_factor);//7-28-202 strangee build error
/****************************************************************************************
...
void canonical_machine_reset(cmMachine_t *_cm)
{
    ...
    // Set fourth axis flag
#ifdef DEPLOY_FOURTH_AXIS
    _cm->fourth_axis_enabled = true;
#else
    _cm->fourth_axis_enabled = false;
#endif
    ...
    _cm->vfd_comms_established_flag=false;//sme: 3-22-2021
 ...
 * cm_get_probe_state()
 */
#ifdef DEBUG_JOGGING_RUN_AWAY_DEFECT
cmJobKillState  cm_get_job_kill_state(void) {return cm->job_kill_state;}
bool  cm_job_kill_state_requested(void) 
{
  bool result=(cm->job_kill_state==JOB_KILL_JOG_REQUESTED);
  if (result==true)
  {
...
cmProbeState    cm_get_probe_state()   
{ 
    cmProbeState result =cm->probe_state[0];
    return result;
}
bool cm_motion_ended(void){return (cm->motion_state !=MOTION_RUN);}//sme: 6-2-2021 useful for ATC state machine
/*
...
void cm_set_model_linenum(int32_t linenum)
{
    if ((linenum < 0) || (linenum > MAX_LINENUM)) {
        linenum = 0;
        rpt_exception(STAT_INPUT_VALUE_RANGE_ERROR, (char*)"line number > 2B or negative; set to zero");
    }
...
{
#define BAD_LINENO_MAX_COUNT 100//arbitrary
volatile static int  bad_line_num_counter=0;
#define ERRMSG_LEN 80
static char err_msg[ERRMSG_LEN];
    if (cm->gmx.last_line_number+1 != cm->gm.linenum) 
     {
         bad_line_num_counter++;
         if(bad_line_num_counter>BAD_LINENO_MAX_COUNT)
         {
            bad_line_num_counter=-1;
         }
        sprintf(err_msg, "Line Num out_of_seq: prev_linenum: %d vs line present line_num %d\n",
                cm->gmx.last_line_number,cm->gm.linenum);
        rpt_exception(STAT_LINE_NUMBER_OUT_OF_SEQUENCE,err_msg);
...
float cm_get_absolute_position(const GCodeState_t *gcode_state, const uint8_t axis)
{
  ...
  if (gcode_state == MODEL) 
    ...
    result =mp_get_runtime_absolute_position(mr, axis);
    return (result );
}
...
{
    if (!nv->value_int) 
    {                       // if false, reset the matrix and return
...
void cm_set_model_target(const float target[], const bool flags[])
{
    ...
    for (axis=AXIS_A; axis<=AXIS_A; axis++) 
    {
        if (!flags[axis] || cm->a[axis].axis_mode == AXIS_DISABLED) 
        {
            continue;        // skip axis if not flagged for update or its disabled
...
void cm_reset_position_to_absolute_position(cmMachine_t *_cm)
{
    ...
    for (uint8_t axis = AXIS_X; axis < AXES; axis++) 
    {     
        position = mp_get_runtime_absolute_position(_mp->mr, axis);       
        cm_set_position_by_axis(axis,position);        
    }
}
...
stat_t cm_straight_traverse(const float *target, const bool *flags, const cmMotionProfile motion_profile)
{
    ...
    if (status == STAT_MINIMUM_LENGTH_MOVE) 
    {
       ...
       {  // handle condition where zero-length move is last or only move
                             ...
                             const bool flags[])                // all false if no intermediate move
{
    ...
    if (cm->gm.units_mode == INCHES) 
    {
        ...
        for (uint8_t i=0; i<AXIS_A; i++)             
        {                  // Only convert linears (not rotaries)
...
stat_t cm_set_feed_rate( float feed_rate)//sme: 10-26-2022: fix the startup spurious sending of STAT_FEEDRATE_NOT_SPECIFIED
{
    if (feed_rate>FEEDRATE_MAX)
    {
      feed_rate=FEEDRATE_MAX;
    }
    if (cm->gm.feed_rate_mode == INVERSE_TIME_MODE) 
    {
        ...
        //sme: 9-9-2021: added because feedrate can be set in a gcode program independent of an axis designation, and because of that, up to this point, it has not been range checked.
...
stat_t cm_set_feed_rate_mode(/*const sme: 9-9-2021 want to range correct this*/ uint8_t mode)
{
...
#if 0//12-20-2022
    cm->gmx.mfo_factor_updated_count=0;
#endif
}
...
#if 1//7-28-2022ndef REFACTORED_OVERRIDES
/****************************************************************************************
 * cm_fro_control() - M50 feed rate override control
 * cm_tro_control() - M50.1 traverse override control
 *
...
stat_t cm_fro_control(const float P_word, const bool P_flag) // M50
{
    ...
    if (cm->gmx.m48_enable) {               // if master enable is ON
        if (new_enable && (new_override || !cm->gmx.mfo_enable)) 
        {   // 3 cases to start a ramp
            mp_start_feed_override(FEED_OVERRIDE_RAMP_TIME, cm->gmx.mfo_factor);
        } 
        else if (cm->gmx.mfo_enable && !new_enable) 
        {              // case to turn off the ramp
...
stat_t cm_tro_control(const float P_word, const bool P_flag) // M50.1
{
    ...
    if (cm->gmx.m48_enable) 
    {               // if master enable is ON
        if (new_enable && (new_override || !cm->gmx.mfo_enable)) 
        {   // 3 cases to start a ramp
            mp_start_traverse_override(FEED_OVERRIDE_RAMP_TIME, cm->gmx.mto_factor);
        } 
        else if (cm->gmx.mto_enable && !new_enable) 
        {              // case to turn off the ramp
 ...
 *
 * cm_program_end() implements things slightly differently (1a, 8):
...
static void _exec_program_finalize(float* value, bool* flag) 
{
    ...
    {
#if 1//sme: 12-22-2022 FRO was getting reset before the program was ended, so we move it here
        cm_reset_overrides();
#endif
#ifdef ELIMINATE_UNNEC_SPINDLE_OFF_CMDS
        if (spindle_is_running()==true)
#endif
...
#if 0//12-22-2022 sme: move this into _exec_program_finalize because is resets fro before program ends       
...
stat_t cm_ack_estop(nvObj_t *nv)
{
    ...
    nv->value/*edge _flt*/ = (float)cm1.estop_state;
...
static int8_t _coord(nvObj_t *nv)   // extract coordinate system from 3rd character
{
    ...
    return (max ((atoi(ptr)-53), -1));  // return G54-G59 as 0-5, error as -1 SME:I observe g58-53=5, so range would be 0-6, or the value subtracted should be 54, not 53
}
...
static const char msg_cycs5[] = "ATC";//sme: 5-25-2021 reports use this array 
static const char *const msg_cycs[] = { msg_cycs0, msg_cycs1, msg_cycs2, msg_cycs3,  msg_cycs4,msg_cycs5 };
...
static const char msg_safe_dummy[] = "Not used";

static const char msg_safe15[] = "VFD Modbus Comms Timeout";//sme: 4-9-2021
static const char msg_safe31[] = "VFD Drive Tripped";//sme: 4-9-2021
static const char *const msg_safe[] = 
{ 
  msg_safe0, 
  msg_safe1, 
  msg_safe2, 
  ...
  msg_safe_dummy,
   msg_safe_dummy, 
  msg_safe31    
};//sme: 4-9-2021 added msg_safe6, msg_safe9
...
stat_t cm_get_frmo(nvObj_t *nv) { return(_get_msg_helper(nv, msg_frmo, cm_get_feed_rate_mode(ACTIVE_MODEL)));}
 ...
 stat_t cm_get_safe(nvObj_t *nv) {
    ...
    uint8_t safety_msg_ndx = 0;//sme: 3-10-2022 must not go over array boundary
    if ((cm1.safety_state & SAFETY_INTERLOCK_MASK) != 0) 
    {
        safety_msg_ndx |= SAFETY_INTERLOCK_OPEN;//==0x1;
    }
   
    if ((cm1.safety_state & SAFETY_ESC_MASK) != 0)        
    {
        safety_msg_ndx |= SAFETY_ESC_OFFLINE ;//==0x2;
    } 
    
    return (_get_msg_helper(nv, msg_safe, safety_msg_ndx));
}
stat_t cm_get_estp(nvObj_t *nv) 
{ 
    stat_t status=_get_msg_helper(nv, msg_estp, (cm1.estop_state &  (ESTOP_PRESSED|ESTOP_UNACKED)/*==0x3*/));
    return  status; 
}
...
stat_t cm_set_home(nvObj_t *nv) { return (set_int_u8(nv, ((uint8_t &)(cm->homing_state)), false, true)); }
...
{ 
    volatile int32_t value = get_integer(nv, cm->probe_input);
    if (value ==PROBE_WAITING )
    {
        __NOP();
    }
    return value; 
}
stat_t cm_set_probe_input(nvObj_t *nv) { return (set_int_u8(nv, cm->probe_input, 0, D_IN_CHANNELS)); }
 //sme: work-around against using unused coord offset C axis for user data storage
#define USER_DATA_SAVED_IN_C_AXIS_NDX 5
static float user_data_stored_in_c_axis = 0;

 
stat_t cm_get_coord(nvObj_t *nv) 
{ 
#if 1//sme: due to user data stored in unused C axis, we do thus:
  stat_t status= STAT_OK;
  int coord_ndx=_coord(nv);
  int axis_ndx =_axis(nv);
  if (axis_ndx <AXES)
  {  
      status= get_float(nv, cm->coord_offset[ coord_ndx][axis_ndx]);
  }
  else
  {
     if (axis_ndx != USER_DATA_SAVED_IN_C_AXIS_NDX)
     {
     ...
     {
       nv->valuetype=TYPE_FLOAT;
        nv->value = user_data_stored_in_c_axis;
     }    
  }
  return status;
#else
  return (get_float(nv, cm->coord_offset[_coord(nv)][_axis(nv)])); 
#endif
}
stat_t cm_set_coord(nvObj_t *nv) 
{ 
#if 1
   stat_t status= STAT_OK;
  int coord_ndx=_coord(nv);
  ...
  if (axis_ndx <AXES)
  {
     status = set_float(nv, cm->coord_offset[coord_ndx][axis_ndx]); 
  }
  else
  {
    if (axis_ndx != USER_DATA_SAVED_IN_C_AXIS_NDX)
    ...
    {
      user_data_stored_in_c_axis =nv->value_flt;
    }
  }
  return status;
#else
  return (set_float(nv, cm->coord_offset[_coord(nv)][_axis(nv)])); 
#endif
}
...
stat_t cm_set_hi(nvObj_t *nv) { return (set_int_u8(nv, cm->a[_axis(nv)].homing_input, 0, D_IN_CHANNELS)); }
...
stat_t cm_set_hd(nvObj_t *nv) 
{
    ...
    uint8_t axis_ndx=_axis(nv);
    result = set_int_u8(nv, cm->a[axis_ndx].homing_dir, 0, 1);
    return result ;
}
...
stat_t cm_get_las(nvObj_t *nv) { return (get_integer(nv, cm->a[_axis(nv)].latch_active_state)); }
stat_t cm_set_las(nvObj_t *nv) 
{
    stat_t result=STAT_OK;
    uint8_t axis_ndx=_axis(nv);
    result = set_int_u8(nv, cm->a[axis_ndx].latch_active_state, 0, 1);
    if (result==STAT_OK)
    {
        motion_set_limit_sw_active_state(axis_ndx,
                cm->a[axis_ndx].latch_active_state);
    }
    return result;
...
stat_t cm_set_sl(nvObj_t *nv) { return(set_int_u8(nv, (uint8_t &)cm->soft_limit_enable, 0, 1)); }

stat_t cm_get_lim(nvObj_t *nv) { return(get_integer(nv, cm->limit_enable)); }
stat_t cm_set_lim(nvObj_t *nv) 
{ 
  return(set_int_u8(nv, (uint8_t &)cm->limit_enable, 0, 1)); 
}

stat_t cm_get_saf(nvObj_t *nv) { return(get_integer(nv, cm->safety_interlock_enable)); }
stat_t cm_set_saf(nvObj_t *nv) { return(set_int_u8(nv, (uint8_t &)cm->safety_interlock_enable, 0, 1)); }

stat_t cm_get_m48(nvObj_t *nv) { return(get_integer(nv, cm->gmx.m48_enable)); }
stat_t cm_set_m48(nvObj_t *nv) { return(set_int_u8(nv, (uint8_t &)cm->gmx.m48_enable, 0, 1)); }

stat_t cm_get_froe(nvObj_t *nv) { return(get_integer(nv, cm->gmx.mfo_enable)); }
stat_t cm_set_froe(nvObj_t *nv)
{ 
    stat_t result = set_int_u8(nv, (uint8_t &)cm->gmx.mfo_enable, 0, 1); 
    return result;
}
stat_t cm_get_fro(nvObj_t *nv)  
{ 
    float result=get_float(nv, cm->gmx.mfo_factor);
    return(result); 
}
...
{ 
    stat_t result =0;        
    result=set_float_range(nv, cm->gmx.mfo_factor, FEED_OVERRIDE_MIN, FEED_OVERRIDE_MAX);  
    return result;
}

stat_t cm_get_troe(nvObj_t *nv) { return(get_integer(nv, cm->gmx.mto_enable)); }

stat_t cm_set_troe(nvObj_t *nv) 
{ 
    stat_t result = set_int_u8(nv, (uint8_t &)cm->gmx.mto_enable, 0, 1);
    return result; 
}
...
{ 
    ...
    stat_t result = STAT_OK;
#if 1//sme:12-21-2022: s/w is copying to TRO the values it receives for fro, and fro max is 2.0 whereas tro max is 1.0 
    if (nv->value>TRAVERSE_OVERRIDE_MAX)
    {
       nv->value=TRAVERSE_OVERRIDE_MAX; 
    }
#endif
    result = set_float_range(nv, cm->gmx.mto_factor, TRAVERSE_OVERRIDE_MIN, TRAVERSE_OVERRIDE_MAX);
    return result; 
}

stat_t cm_get_gpl(nvObj_t *nv) { return(get_integer(nv, cm->default_select_plane)); }
stat_t cm_set_gpl(nvObj_t *nv) { return(set_int_u8(nv, (uint8_t &)cm->default_select_plane, CANON_PLANE_XY, CANON_PLANE_YZ)); }

stat_t cm_get_gun(nvObj_t *nv) { return(get_integer(nv, cm->default_units_mode)); }
stat_t cm_set_gun(nvObj_t *nv) { return(set_int_u8(nv, (uint8_t &)cm->default_units_mode, INCHES, MILLIMETERS)); }

stat_t cm_get_gco(nvObj_t *nv) { return(get_integer(nv, cm->default_coord_system)); }
stat_t cm_set_gco(nvObj_t *nv) { return(set_int_u8(nv, (uint8_t &)cm->default_coord_system, G54, G59)); }

stat_t cm_get_gpa(nvObj_t *nv) { return(get_integer(nv, cm->default_path_control)); }
stat_t cm_set_gpa(nvObj_t *nv) { return(set_int_u8(nv, (uint8_t &)cm->default_path_control, PATH_EXACT_PATH, PATH_CONTINUOUS)); }

stat_t cm_get_gdi(nvObj_t *nv) { return(get_integer(nv, cm->default_distance_mode)); }
stat_t cm_set_gdi(nvObj_t *nv) { return(set_int_u8(nv, (uint8_t &)cm->default_distance_mode, ABSOLUTE_DISTANCE_MODE, INCREMENTAL_DISTANCE_MODE)); }

stat_t cm_get_four(nvObj_t *nv)  { return (get_integer(nv, cm->fourth_axis_enabled)); }
...
stat_t cm_dam(nvObj_t *nv)
{
    ...
    comms_mgr_write_msg((char*)"Active model:\n");
    cm_print_vel(nv);
    cm_print_feed(nv);
    cm_print_line(nv);
    cm_print_stat(nv);
...
#ifdef REFACTOR_LIMIT_SWITCHES
static const char fmt_Xlas[] = "[%s%s] %s homing switch Active State %11d [0=NO , 1=NC ]\n";
#endif
...
#if  1//sme: 5-21-2021 prevent reset
static const char fmt_hom[] = "%c axis homing state:%d\n";
#else
...
void cm_print_aatcp(nvObj_t *nv)
{
  sprintf(cs.out_buf, "[%s%s] Starts ATC Pickup state machine\n",nv->group, nv->token);
  comms_mgr_write_msg(cs.out_buf) ;
}
...
#ifdef REFACTOR_LIMIT_SWITCHES 
void cm_print_las(nvObj_t *nv) { _print_axis_ui8(nv, fmt_Xlas);}
#endif
...
static const char fmt_four[]  = "Fourth axis deployed = %d [0=disable,1=enable]\n";
void cm_print_four(nvObj_t *nv) { text_print(nv, fmt_four);}
