...
stat_t mp_aline(GCodeState_t* _gm)
{
    ...
    volatile float length_square = 0;
...
//    if (length < 0.00002) {  // this value is 2x EPSILON and prevents trap failures in _plan_aline()
    if (length < 0.0001) 
    {      // this value is 0.1 microns. Prevents planner trap failures
    ...
    {       // compute the unit vector and set flags
        if ((bf->axis_flags[axis] = flags[axis])) 
        {     // yes, this is supposed to be = and not ==
...
static mpBuf_t* _plan_block(mpBuf_t* bf)
{
    ...
    if (mp->planner_state == PLANNER_PRIMING) 
    {
        ...
        if (bf->nx->plannable) 
	    {                        // read in new buffers until EMPTY
            return (bf->nx);
        }
    ...
    if (mp->planner_state == PLANNER_BACK_PLANNING) 
    {
        ...
        for (; bf->plannable || (braking_velocity < bf->exit_velocity); bf = bf->pv) 
	    {
            ...
            else 
	        {
                ...
                if (bf->pv->exit_vmax > braking_velocity) 
		        {  // remember, exit vmax already is min of
...
static void _calculate_jerk(mpBuf_t* bf)
{
    ...
    for (uint8_t axis = 0; axis < AXES; axis++) 
    {
        if (std::abs(bf->unit[axis]) > 0) 
        {  // if this axis is participating in the move
...
static void _calculate_vmaxes(mpBuf_t* bf, const float axis_length[], const float axis_square[])
{
    ...
    if (bf->gm.motion_mode != MOTION_MODE_STRAIGHT_TRAVERSE) 
    {
        ...
        else 
        {
            ...
            {
                feed_time = sqrt(axis_square[AXIS_A]
                        //+ axis_square[AXIS_B] 
                        //+ axis_square[AXIS_C]
                        ) 
                        / bf->gm.feed_rate;       
    ...
    {
       if(bf->absolute_vmax>JOGGING_MAX_VELOCITY)
       {
         bf->absolute_vmax=JOGGING_MAX_VELOCITY;//8-30-2021 
       }
    }
 ...
 *
 *    C) The last move (there is not "next move" yet) will have to compute to a unit vector of zero.
 */
...
static void _calculate_junction_vmax(mpBuf_t* bf)
{
    ...
    for (uint8_t axis = 0; axis < AXES; axis++) 
    {
        if (bf->axis_flags[axis] || bf->nx->axis_flags[axis]) 
        {       // (A) skip axes with no movement
