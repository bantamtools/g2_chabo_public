...
#include "comms_mgr.h"//usbmr_flushRead
#include "g2_util.h"
#include "g2_report.h"
...
{
    if (cm1.job_kill_state == JOB_KILL_REQUESTED) 
    {         // job kill must wait for any active hold to complete
        _start_job_kill();
    }

    if (cm1.queue_flush_state == QUEUE_FLUSH_REQUESTED)
    {   // look for a queued flush request
        _start_queue_flush();
    }
    if (cm1.cycle_start_state == CYCLE_START_REQUESTED) 
    {   // look for a queued cycle start ot restart
...
{
    if (cm1.hold_state != FEEDHOLD_OFF) 
    {   // restart from a feedhold
        if (cm1.queue_flush_state == QUEUE_FLUSH_REQUESTED) 
        {   // possible race condition. Flush wins
...
void _start_queue_flush()
{
    ...
    {
       //sme: flush pending incoming command (i.e. gcode) msgs
        comms_mgr_flushRead();
...
//    coolant_control_immediate(COOLANT_OFF, COOLANT_BOTH); // stop coolant
    ...
    rpt_exception(STAT_KILL_JOB, (char*)"Job killed by ^d");
...
void _start_job_kill()
{
    switch (cm1.machine_state) {
        ...
        case MACHINE_CYCLE: {                           // Case 2's
            if (cm1.hold_state == FEEDHOLD_OFF) 
            {       // Case 2a - in cycle and not in a hold
                op.add_action(_feedhold_no_actions);
            }
            if (cm1.hold_state == FEEDHOLD_HOLD) 
            {     
                _run_job_kill();                        // Case 2c - in a finished hold
            }
...
stat_t _feedhold_skip()
{
    ...
    volatile static int feedhold_state_ltms_counter=0;
#endif
    ...
    {
#ifdef DEPLOY_PROBE_FEEDHOLD_TIMEOUT
       feedhold_state_ltms_counter++;
       if (probe_check_feedhold_timeout()==true)
       {
          //send error report but continue
           rpt_exception( STAT_PROBE_FEEDHOLD_TIMEOUT,(char*)"Forcing completion of Probe Feedhold state machine");
       }
       else
#endif
        return (STAT_EAGAIN);
...
#ifdef DEPLOY_PROBE_FEEDHOLD_TIMEOUT//12-19-2022 track normal completion of feedhold
     cycle_probing_feedhold_off();   
#endif   
...
stat_t _feedhold_with_actions()          // Execute Case (5)
{
    ...
    if (cm1.hold_state == FEEDHOLD_MOTION_STOPPED) 
    {
        ...
        {// optional Z lift
            ...
            {  // if the value is negative, we want to go to Z-max position with G53
                if (cm->homed[AXIS_Z]) 
                {    // ONLY IF HOMED
...
stat_t _feedhold_restart_with_actions()   // Execute Cases (6) and (7)
{
    ...
    if (cm1.hold_state == FEEDHOLD_HOLD) {
        if ((spindle_is_on_or_paused() && !spindle_ready_to_resume())) {
            return (STAT_EAGAIN);
        }
...
//        coolant_control_sync(COOLANT_RESUME, COOLANT_BOTH); // resume coolant if paused
        ...
        // do return move through an intermediate point; queue a wait
