...
//10-20-2021 Mnemonic replaces a literal 128 size limit on rpt_err message buff that also had no range checks, resulting in cpu resets when message was longer than 128 bytes
#define EXCP_RPT_MSG_MAX_LEN 512
...
/* sme:Motate::Timeout*/ uint32_t status_report_systick; // SysTick value for next status report
    ...
    double status_report_value[NV_STATUS_REPORT_LEN];    // previous values for filtered reporting
#else    
...
void rx_request_rx_report(void);//sme: not deployed!
...
#if 1//SME: experiment: see whether report during probing are interfering with RX
 void debug_reset_prbr_instance_count(void);
#endif
#ifdef __TEXT_MODE
