...
/*
 * g2_canonical_machine.h - rs274/ngc canonical machining functions
...
#define JOGGING_MAX_VELOCITY ((float)2000.0)//SME--replace hard-coded literal with mnemonic
...
typedef enum {
    ...
    CYCLE_ATC_SEEK,                 // sme: special homing along far end of x axis in ATC tool carriage pickup or return
...
typedef enum {                      // state machine for cycle start
    ...
    CYCLE_START_COMPLETE//sme: never used
} cmCycleState;
...
typedef enum {                      // applies to cm->homing_state
    ...
    HOMING_WAITING   //sme: never used // machine waiting to be homed
} cmHomingState;
...
//7-21-2021: not in latest g2core commit
 typedef enum {
...
typedef enum 
{
    ...
    SAFETY_ESC_ONLINE       = SAFETY_INTERLOCK_CLOSED,
    SAFETY_INTERLOCK_OPEN   = 0x01, 
    SAFETY_ESC_OFFLINE      = 0x02,            
    SAFETY_ESC_LOCKOUT      = 0x04,
    SAFETY_ESC_REBOOTING    = 0x08,
    SAFETY_ESC_LOCKOUT_AND_REBOOTING = (SAFETY_ESC_LOCKOUT|SAFETY_ESC_REBOOTING),            
    SAFETY_INTERLOCK_MASK = SAFETY_INTERLOCK_OPEN,         
    SAFETY_ESC_MASK       = (SAFETY_ESC_OFFLINE|SAFETY_ESC_LOCKOUT|SAFETY_ESC_REBOOTING)//was 0x0E,//0000 1110  ==rebooting|lockout|offline            
} cmSafetyState;
...
typedef struct cmAxis {
    ...
    uint8_t latch_active_state;              // homing limit switch active state: NO=0, NC=1
#endif
} cfgAxis_t;
...
typedef struct cmMachine {                  // struct to manage canonical machine globals and state
  ...
  /**** Runtime variables (PRIVATE) ****/
    ...
    bool vfd_comms_established_flag;        // sme: 3-22-2021
    bool vfd_alarm_tripped_flag;            // sme: 4-9-2021
    ...
    bool fourth_axis_enabled;               // Fourth axis deployed
    magic_t magic_end;
} cmMachine_t;
...
bool cm_probe_cycle_active(void);//sme: 4-23-2021 --for sol4 pwm operation
// Model state getters and setters
#ifdef DEBUG_JOGGING_RUN_AWAY_DEFECT
cmJobKillState  cm_get_job_kill_state(void); 
bool cm_job_kill_state_requested(void) ;
#endif
#ifdef DEBUG_CM_STRUCT_ACCESS_ANOMALY 
void test_cm_struct_access(void);
#endif
...
bool cycle_homing_active(void);//sme 5-3-2021 supposrts stallguard tuning
float cycle_get_homing_velocity(void);//sme 5-3-2021 supposrts stallguard tuning
// Coordinate systems and offsets
...
#if 0//sme: struct has P and  words as floats!
stat_t cm_set_g10_data(const float P_word, const bool P_flag,             // G10
                       const float L_word, const bool L_flag,
...
bool cm_motion_ended(void);//sme: added for ATC cycle 6-2-2021
...
#ifdef DEBUG_JOGGING_RUN_AWAY_DEFECT 
bool cm_hold_stable(void);
void set_jobkill_to_stop_jog(void);
#endif
// has hold in primary planner
...
stat_t cm_start_atcp(nvObj_t *nv);      // sme: 5-24-2021 start the ATC tool acarriage pickup sequence
...
stat_t cm_get_four(nvObj_t *nv);         // get if fourth axis deployed

bool atc_homing_cycle_active(void);
...
/*--- text_mode support functions ---*/
...
#ifdef REFACTOR_LIMIT_SWITCHES 
     void cm_print_las(nvObj_t *nv);
#endif      
    void cm_print_aatcp(nvObj_t *nv);//sme 5-24-2021
    void cm_print_cofs(nvObj_t *nv);
    void cm_print_cpos(nvObj_t *nv);

    void cm_print_four(nvObj_t *nv);
    ...
    #define cm_print_aatcp tx_print_stub//sme 5-24-2021
    void cm_print_cofs(nvObj_t *nv);
    void cm_print_cpos(nvObj_t *nv);
    ...
    #define cm_print_four tx_print_stub
...
#endif
    extern bool probing_in_progress(void);//sme 11-9-2022
