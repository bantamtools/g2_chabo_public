...
/*
 * g2_stepper.h - stepper motor interface
 ...
 *      causes _exec_move() to run at the MEDium interrupt level.
 *
...
#include "g2core.h"
#include "g2_report.h"
#include "g2_planner.h"    // planner.h must precede stepper.h for moveType typedef
 ...
 */
#define MIN_STEP_CORRECTION_FACTOR 0.0
#define MAX_STEP_CORRECTION_FACTOR 5.0
...
#if 1//sme: debug only: V3 does not support motor timeout
...
#else //original worried about alignment
    uint8_t  motor_map;                     // map motor to axis
    uint32_t microsteps;                    // microsteps to apply for each axis (ex: 8)
...
typedef struct stPrepMotor 
{
 ...
 //sme: Not used float prev_segment_time;                // segment time from previous segment run for this motor
 //sme: Not used float accumulator_correction;           // factor for adjusting accumulator between segments
 //sme: Not used uint8_t accumulator_correction_flag;    // signals accumulator needs correction
} stPrepMotor_t;
...
extern float st_step_correction_factor; 
extern float st_step_correction_max;
extern void st_print_scscalef(nvObj_t *nv);
extern stat_t st_set_scscalef(nvObj_t *nv);
/**** Stepper (base object) ****/
...
public:
    ...
    virtual void stepStart()  { /* must override */ }; // HOT - called from the DDA interrupt
    virtual void stepEnd()  { /* must override */ };   // HOT - called from the DDA interrupt
    virtual void setDirection(uint8_t direction)  { /* must override */ }; // HOT - called from the DDA interrupt
...
void st_init_motors(void);//4-6-2022--strange linker error workaround
 ...
 * Stepper configs and constants *
 *********************************/
#define STEPPER_SUBSTEP_INCREMENT_IS_64BIT
#define STEPPER_MICROSTEP_IS_16BIT
