/*
 * g2_gcode.h - rs274/ngc Gcode model and parser support
...
#include "g2_config.h"
#define RX_BUFFER_SIZE       1024//11-13-2020 512            // maximum length of recieved lines from xio_readline
...
typedef enum {
    INVERSE_TIME_MODE = 1,//sme 10-26-2022 eliminate error 142 was:0,   // G93
...
//sme: 3-23-2023: to do : consolidate this enum with that defined in user_types: DirectionE;
...
GpioHandlerStatusE _homing_handler (void);/* Gcode state structures */
...
typedef struct GCodeState {             // Gcode model state - used by model, planning and runtime
...
#if 1//sme: DID NOT HELP!!!! 9-10-2021 try teasing out a strange defect
    uint8_t feed_rate_mode;
#else
    ...
    uint8_t spindle_direction;           // M3/M4/M5 - spindle on CW, on CCW, off setting
...
#if 0//12-20-2022 mfo is getting overwritten by regular gcode feed rate commands
    int mfo_factor_updated_count;
    float mfo_factor_saved;
#endif
...
float get_feedrate(void);//sme: 4-30-2021: stallguard status streaming uses feedrate
