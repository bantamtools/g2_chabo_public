...
#include "g2_canonical_machine.h"    // used for GCodeState_t
#include "main.h"            
...
typedef enum {                      // bf->buffer_state values in increasing order so > and < can be used
...
#ifdef DEBUG_SKIPPED_ALINE_DURING_PROBE
#define MAX_BUF_ENTRIES_TRACKED 50//Arbitrary
#define MAX_ISR_ENTRIES_TRACKED 200//Arbitrary
...
typedef struct
{
 short fwd_plan_entry_count;
 short plan_exec_entry_count;
 short write_buf_nbr[MAX_BUF_ENTRIES_TRACKED]; 
 short read_buf_nbr[MAX_BUF_ENTRIES_TRACKED]; 

 short write_buf_ndx;
 short read_buf_ndx;

 short fwd_plan_isr_cnt; 
 short plan_exec_isr_cnt; 
 BufInfoS fwd_plan_isr_entry[MAX_ISR_ENTRIES_TRACKED];
 BufInfoS plan_exec_isr_entry[MAX_ISR_ENTRIES_TRACKED];
 void clear_log(void)
 {
     memset(&write_buf_nbr,0,sizeof(write_buf_nbr));
     memset(&read_buf_nbr,0,sizeof(read_buf_nbr));
     memset(&fwd_plan_isr_entry, 0,sizeof(fwd_plan_isr_entry));
     memset(&plan_exec_isr_entry,0,sizeof(plan_exec_isr_entry));
 ...
 void log_wbnbr(int bufnbr)
 {
    write_buf_nbr[write_buf_ndx]=bufnbr;
    write_buf_ndx++;
    if(write_buf_ndx>=MAX_BUF_ENTRIES_TRACKED)
       write_buf_ndx=0;
 }
 
 void log_rbnbr(int bufnbr)
 {
    read_buf_nbr[read_buf_ndx]=bufnbr;
    read_buf_ndx++;
    if(read_buf_ndx>=MAX_BUF_ENTRIES_TRACKED)
       read_buf_ndx=0;
 }
 

 void log_fwd_isr_entry(int buf_nbr,
                       bufferState buf_state,
                       blockType block_type,
                       blockHint block_hint,
                       moveSection move_section 
 ...
 {
     fwd_plan_entry_count++;
    fwd_plan_isr_entry[fwd_plan_isr_cnt].buf_nbr=buf_nbr;
    fwd_plan_isr_entry[fwd_plan_isr_cnt].block_hint =block_hint;
    fwd_plan_isr_entry[fwd_plan_isr_cnt].block_type=block_type;
    fwd_plan_isr_entry[fwd_plan_isr_cnt].buf_state=buf_state;
    fwd_plan_isr_entry[fwd_plan_isr_cnt].move_section =move_section;
            
    fwd_plan_isr_cnt++;
    if (fwd_plan_isr_cnt >= MAX_ISR_ENTRIES_TRACKED)
       fwd_plan_isr_cnt=0;
 }
 void log_plan_exec_isr_entry(int buf_nbr,
                       bufferState buf_state,
                       blockType block_type,
                       blockHint block_hint,
                       moveSection move_section )
 {
     plan_exec_entry_count++;
    plan_exec_isr_entry[ plan_exec_isr_cnt ].buf_nbr=buf_nbr;
    plan_exec_isr_entry[ plan_exec_isr_cnt ].block_hint=block_hint;   
    plan_exec_isr_entry[ plan_exec_isr_cnt ].block_type= block_type;
    plan_exec_isr_entry[ plan_exec_isr_cnt ].buf_state=buf_state;
     plan_exec_isr_entry[ plan_exec_isr_cnt ].move_section=move_section ;       
    plan_exec_isr_cnt++;
    if (plan_exec_isr_cnt >=MAX_ISR_ENTRIES_TRACKED)
       plan_exec_isr_cnt=0;
 }
}TrackPlanningInterruptsS;

extern TrackPlanningInterruptsS track_planning_ints;
...
lllllll
#define PLANNER_QUEUE_SIZE          ((uint8_t)200//sme: 11-28-2022 was: 48)       // Suggest 12 min. Limit is 255
#endif
#ifndef SECONDARY_QUEUE_SIZE
#define SECONDARY_QUEUE_SIZE        ((uint8_t)24//sme: 11-28-2022 was: 12)       // Secondary planner queue for feedhold operations
#endif
...
#define FEED_OVERRIDE_ENABLE        true//7-28-2022 false               // initial value
...
#define TRAVERSE_OVERRIDE_ENABLE    true               // initial value
...
#if 1//sme: 11-28-2022 doing FRO we ran out of planning buffers at increase of 100 to 200!    
    uint16_t queue_size;                 // total number of buffers, one-based (e.g. 48 not 47)
    uint16_t buffers_available; 
#else//original 256 upper limit
...
{   // common variables for a planner context
    ...
    // objects
    /*Timeout*/ uint32_t block_timeout; // Timeout object for block planning
...
#if 1//2-12-2023: probe defect debugging
    int write_buf_nbr;
#endif    
    magic_t magic_end;
...
extern json_commands_t *jc; //sme: what is this? compiler doe not get it :HOT_DATA;             // currently active JSON command buffer
...
stat_t mp_set_target_steps(const float target_steps[]);
...
void mp_replan_queue(mpBuf_t *bf, bool back_too=false);//7-28-2022void mp_replan_queue(mpBuf_t *bf);
...
#ifdef DEPLOY_DUST_REMOVAL_SOL2
float mp_get_zpos(void);
#endif
