...
/*
 * g2_util.h - a random assortment of useful functions
...
int my_strcmp(char *str1, char *str2);//sme
...
#define INIT_AXES_ZEROES {0,0,0,0}
#define INIT_AXES_ONES   {1,1,1,1}
...
#endif
//sme: 8-8-2022: issue: epsilon is > min segment time! 0.0000125 minutes! 0.75 msec/60,000 msec/min
#ifndef EPSILON
...
//sme:
uint16_t ByteSwap16(uint16_t raw_16);
uint32_t ByteSwap32(uint32_t raw_32);
/* SME: Define a macro for range checking and correcting a value of any data type. 
 *  sets a flag if correction was made:
*/
 
#define RANGE_CHECK_CORRECT(val,min, max, status) \
if (val < min) {val = min; status=STAT_INPUT_VAL_CORRECTED_TO_MIN; } \
else if (val > max) {val = max; status=STAT_INPUT_VAL_CORRECTED_TO_MAX;} 

/*  SME: Define a macro for range checking that does not correct, only reports out of range */
#define RANGE_CHECK_REJECT(val,min, max, status) \
if (val < min) status=STAT_INPUT_LESS_THAN_MIN_VALUE; \
else if (val > max) status=STAT_INPUT_EXCEEDS_MAX_VALUE; 
